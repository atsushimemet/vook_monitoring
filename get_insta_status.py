...
column_list = ['endtime', 'follower_count', 'profile_views', 'impressions', 'reach', 'follower']
df_account_st_main = pd.concat([df_file_latest,df_account_status_in30days], ignore_index=True)
...
follower_today = user_info(business_account_id=INSTAGRAM_ACCOUNT_ID,token=ACCESS_TOKEN,username=username,fields=fields)[ 'followers_count']
...
df_account_st_main.to_csv('../data/output/account_status_vook_'+today+'.csv', index=False)
...
list_column2 = ['endtime','follower_count','impressions','profile_views','reach','follower']
